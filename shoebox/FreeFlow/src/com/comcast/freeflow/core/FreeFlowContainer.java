...
public class FreeFlowContainer extends AbsLayoutContainer {
    ...
    private double finger_touch_tap_distance_torlance = 10d;
    /**
     ...
     *
     * @param w Width of the viewport. Since right now we don't support
     *          margins and padding, this is width of the container.
     * @param h Height of the viewport. Since right now we don't support
     ...
     * @param freeflowItem The <code>FreeFlowItem</code> instance that represents the
    ...
    protected void doLayout(FreeFlowItem freeflowItem) {
        ...
        view.layout(frame.left - viewPortX, frame.top - viewPortY,
                frame.right - viewPortX, frame.bottom - viewPortY);
    }
    ...
    /**
     * give a setting to change the finger touch allanace distance. it is the distance from the touch start to touch end distance.
     *
     * @param allowance
     */
    public void setFinger_touch_tap_distance_torlance(double allowance) {
        finger_touch_tap_distance_torlance = allowance;
    }
    ...
    protected void computeViewPort(FreeFlowLayout newLayout) {
        ...
        /**
         * @todo: take a look of the code in here... try to make it work and less jerky
         */
     ...
     *
     * @param dataItem The data object being rendered in a View managed by the
    ...
    protected double getDistance(float xDiff, float yDiff) {
        return Math.sqrt(xDiff * xDiff + yDiff * yDiff);
    }
    ...
    protected void touchUp(MotionEvent event) {
        if ((mTouchMode == TOUCH_MODE_SCROLL || mTouchMode == TOUCH_MODE_OVERFLING)
            ...
            if (beginTouchAt != null && beginTouchAt.view != null && beginTouchAt == endTouchAt) {
                ...
                double distance = getDistance(event.getX(), event.getY(), deltaX, deltaY);
                if (distance < finger_touch_tap_distance_torlance) {
